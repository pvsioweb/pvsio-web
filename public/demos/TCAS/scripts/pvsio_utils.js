...
function TCAS_tick() {
	ws.sendGuiAction("tick(" + (getLastState()) + ");");
}
...
function draw_intruder(positionX_i, positionY_i, positionX_o, positionY_o, ta, range, converging ) {
	...
	positionY_i  =  positionY_i - positionY_o + centerY;
	

	// Draw intruder using one of the following symbols:
    //  - red square, when ta is TRUE
...
var render_pvs_state = function(e) {
	// parse the state and initialise corresponding variables
	pvsio_response = prettyprintPVSioOutput(e.data);
	console.log(pvsio_response);
	var allElements = pvsio_response.split(',');
//	for (var key in allElements) {
//		console.log(allElements[key]);
//		console.log(allElements[key].replace(/[^\d-./]/g, ""));	
//	}
	...
	// Updating Tcas Console
	uiConsole.innerHTML = "[ " + (new Date()).toTimeString() + " ] " + pvsio_response + "<br><br>" + uiConsole.innerHTML;
...
require(["websockets/pvs/pvsWSClient"], function(pvsws){
	...
	ws = pvsws()
		...
		.addListener('ConnectionOpened', function(e){
			ws.startPVSProcess("top", "../demos/TCAS" , function (e) {
