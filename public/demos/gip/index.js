...
define('formal/pvs/prototypebuilder/displayMappings',[], function(){
	...
	mappings.is_on_field = {regex: new RegExp("is_on := [0-9\/.]+")};
	mappings.vtbi_field = {regex: new RegExp("vtbi := [0-9\/.]+")};
	mappings.infusing_field = {regex:new RegExp("is_infusing := [0-9\/.]+")};
	mappings.ctrl_cmd_field = {regex: new RegExp("ctrl_cmd := [0-9A-Za-z]+")};
...
define('formal/pvs/prototypebuilder/displayManager',[ 'd3/d3'], function(){
	var splitter = ":= ";
	var o = {};
	o.updateDisplay = function(stateString, mappings){
		if(stateString){
			...
			for(key in mappings){
				...
				if(uiElement){
					d3.select("#" + uiElement).html(dispVal);
				}
				uiElement = undefined;
			}
		}//endif
...
/**
 * module for editing mark details
 * @author Patrick Oladimeji
 * @date Dec 5, 2012 : 10:29:16 PM
 */

define('formal/pvs/prototypebuilder/markEditor',['d3/d3'], function(){
	/**
	...
	return function(callback, mark, ws){
		var type, name, funcText, funcRowLabel = "Function:";
		if(mark && mark.attr("id")){
			var id = mark.attr("id");
			type = allMaps[id].type;
			name  = allMaps[id].name;
			funcText = allMaps[id].functionText;
		}
		...
		var widgetType = typerow.append("td").append("select")
			.on("change", function(d){
				...
				funcRowLabel = d === "Button" ? "Function:" : "Regex:";
			});
		...
		namerow.append("td").append("label").html("Widget Name:");
		var widgetName = namerow.append("td").append("input").attr("type", "text");
		if(name)
			widgetName.property("value", name);
		
		var funcrow = table.append("tr");
		funcrow.append("td").append("label").html(funcRowLabel);
		var functionDetails = funcrow.append("td").append("input").attr("type", "text");
		if(funcText)
			functionDetails.property("value", funcText);
		var okcancelrow = table.append("tr");
		...
		okcancelrow.append("td").append("input")
			.attr("type", "button").attr("value", "OK").on("click", function(){
				...
				if(funcRowLabel === "Function:"){
					...
					displayMappings[name]  = {regex :new RegExp(funcText), uiElement:name};
				}
...
define('formal/pvs/prototypebuilder/createOverlay',['./markEditor', 'd3/d3'], function(editMark){
	
	return function(parent, formHandler, ws){
		var mx = d3.event.clientX, my = d3.event.clientY;
		...
		return parent.append("div").style("left", mx + "px")
			.style("top", my + "px")
			.attr("startx", mx)
			.attr("starty",  my)
			...
			.on("mousedown", function(){
				...
				editMark(formHandler, d3.select(this), ws);
			});
	};
});
/**
...
require(['websockets/pvs/pvsiowebsocket','formal/pvs/prototypebuilder/displayManager',
         'formal/pvs/prototypebuilder/displayMappings','formal/pvs/prototypebuilder/createOverlay',
         'formal/pvs/prototypebuilder/markEditor', 'd3/d3'], 
	function(pvsws, displayManager, displayMappings, createOverlay, editMark){
	var allMaps = {};
	...
	var ws = pvsws()
		.addListener('ConnectionOpened', function(e){
			...
			var response = (e.data.toString().replace(/,,/g,','));
			console.log(response);
			pvsio_response_log(response);
			displayManager.updateDisplay(response, displayMappings);
	...
	d3.select("#btnToggleOverlay").on("click", function(){
		var layer = d3.select("img");
		if(layer.style("z-index") === "-2"){
...
//	 * @param callback is a function to call when all the details have been supplied by the user
//	 */
//	function requestMarkDetails(callback, mark){
//		var type, name, funcText, funcRowLabel = "Function:";
//		if(mark && mark.attr("id")){
//			var id = mark.attr("id");
//			type = allMaps[id].type;
...
//	
	 ...
	 */
	function handleFormDetails(type, name, functionDetails){
		allMaps[name] = {name:name, type:type, functionText: functionDetails};
...
//		if(!d3.selectAll(".mark.selected").empty())
//			d3.selectAll(".mark.selected").classed("selected", false);
//		return parent.append("div").style("left", mx + "px").style("top", my + "px")
//			.attr("startx", mx).attr("starty",  my).attr("class", "mark selected")
//			.on("mousedown", function(){
//				d3.event.preventDefault();
//				d3.event.stopPropagation();
//				startMouseX = d3.event.clientX;
...
//				}
//			}).on("dblclick", function(){
//				requestMarkDetails(handleFormDetails, d3.select(this));
//			});
//	}
});
