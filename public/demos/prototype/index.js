...
require(['websockets/pvs/pvsiowebsocket', 'd3/d3'], function(pvsws){
	var allMaps = {};
	...
	var ws = pvsws()
		.addListener('ConnectionOpened', function(e){
			...
			console.log(e.data.toString().replace(/,,/g,','));
...
//		c.innerHTML = msg + "<br>" + c.innerHTML;
	...
	function requestMarkDetails(callback, mark){
		...
		funcrow.append("td").append("label").html("Function:");
	...
	function createOverlay(parent){
		...
		if(!d3.selectAll(".mark.selected").empty())
			d3.selectAll(".mark.selected").attr("class","mark");
		return parent.append("div").style("left", mx + "px").style("top", my + "px")
			...
			.on("mousedown", function(){
				...
				d3.selectAll(".selected").attr("class", "mark");
				d3.select(this).attr('class', "mark selected");
				moving = true;
			}).on("mouseup", function(){
