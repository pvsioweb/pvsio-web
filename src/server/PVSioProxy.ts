...
class PvsProcess {
	...
	/**
	 * Utility function, returns the raw output of PVSio
	 */
	getData (): string { return this.data; }
	/**
	 * Utility function, resets the raw output of PVSio
	 */
			...
			if (fileExists && !this.pvsioProcess) {
				...
				this.pvsioProcess.stdout.on("data", (data: string) => {
					...
					if (this.data.trim().endsWith(readyString)) {
						// console.log("[pvsio-proxy] PVSio ready!");
						if (!this.ready) {
							this.ready = true;
							resolve(true);
						}
...
/**
 * PVSio response type
 * - state is the parsed state in JSON format
 * - pvsioOut is the raw string produced by PVSio
 * - jsonOut
 * - error is non-null only when an error occurs. The field is a string message indicating the reason for an error.
 */
export type PvsioResponse = { state: {}, pvsioOut?: string, jsonOut?: string, error?: string };

/**
 * PVSio Proxy is the entry point used by the server to communcate with the PVSio process
 */
export class PvsProxy {
	...
	protected lastState: string; // saves the last expression generated by pvsio

	/**
	 * Constructor, creates an instance of the PVSio proxy
	 */
	constructor (opt?: { pvsPath?: string, pvsLibraryPath?: string }) {
		opt = opt || {};
		this.pvsPath = opt.pvsPath || "";
		this.pvsLibraryPath = opt.pvsLibraryPath || "";
		console.log("[pvsio-proxy] Constructor", { pvsPath: this.pvsPath, pvsLibraryPath: this.pvsLibraryPath });
		// this.pvsLibPath = path.join(pvsPath, "lib");
	}
	/**
	 * Starts a PVSio process
	 */
	async start (desc: FileDescriptor, opt?: { lastState?: string }): Promise<boolean> {
		opt = opt || {};
		this.lastState = opt.lastState || "";
		this.pvsFile = desc;
		console.log(`[pvsio-proxy] Starting pvsio (pvsio path=${this.pvsPath})`);
	...
	/**
	 * Sends a PVS expression to PVSio for evaluation, returns the result of the evaluation.
	 */
	async sendCommand (cmd: string, opt?: { useLastState?: boolean }): Promise<PvsioResponse> {
		opt = opt || {};
		let res: PvsioResponse = { state: {} };
		if (!this.proc) {
			console.log("[pvsio-proxy] Starting pvsio with file ", this.pvsFile);
			await this.start(this.pvsFile);
		}
		if (this.proc && cmd) {
			if (opt.useLastState && this.lastState) {
				cmd = cmd.endsWith(";") ? `${cmd.substring(0, cmd.length - 1)}(${this.lastState});` : `${cmd}(${this.lastState});`;
			} else {
				if (cmd === ";") { cmd = `"";`; }
			}
			...
			if (result?.includes("Expecting an expression") || result?.includes("has the wrong type")) {
				res.error = result;
			} else {
				res.pvsioOut = result;
				res.state = stateParser.parse(result);
	...
	/**
	 * Close the connction with PVSio. This command will kill the PVSio process.
	 */
	async close (): Promise<boolean> {
		return await this.proc?.kill();
	}
	/**
	 * Utility function, returns the current context folder used by PVSio.
	 * The pvs files executed in PVSio are located in this folder.
	 */
