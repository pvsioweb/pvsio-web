...
*/
function readUntil(str: string, start: number, cond) {
    let word: string = "";
    let i = start;
    for (; i < str.length; i++) {
...
function isclosebr(s: string): boolean {
    return s.indexOf(")") >= 0;
}

function isopenbr(s: string): boolean {
    return s.indexOf("(") >= 0;
}
...
*/
export function resolve(state, property) {
...
function parseValue(value) {
    ...
    if (value[token.index] === "(") {
        ...
        token = readUntil(value, token.index, function (ch, prev) {
            return wordChar(ch) || num(ch) || (/[\/\.\-\"\'\s\_\:\?\!\@\$\^\&\(\)\%<>\;]/).test(ch);
        });
...
export function parse (state: string): {} {
    return parseValue(state).value;
}

export function isState (str) {
