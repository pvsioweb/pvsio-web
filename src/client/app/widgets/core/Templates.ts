...
<div class="{{type}}_whole_part" style="position:absolute; left:0px; padding-left:{{whole.padding-left}}px; width:{{whole.width}}px; height:{{whole.height}}px; text-align:right; display:inline-flex;">{{#each whole.digits}}
    <div style="border-radius:2px;text-align:center; width:{{../whole.letter-spacing}}px; min-width:{{../whole.letter-spacing}}px; max-width:{{../whole.letter-spacing}}px; font-size:{{font-size}}pt;{{#if selected}} color:{{../whole.background-color}}; background-color:{{../whole.color}}{{else}} color:{{../whole.color}}; background-color:{{../whole.background-color}}{{/if}}">{{val}}</div>{{/each}}
</div>
...
{{#if frac.viz}}<div class="{{type}}_frac_part" style="position:absolute; left:{{frac.left}}px; width:{{frac.width}}px; height:{{frac.height}}px; text-align:left; display:inline-flex;">{{#each frac.digits}}
