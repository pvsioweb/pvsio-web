...
export class WidgetEditor extends Backbone.View {
    ...
    protected samples: { [key:string]: WidgetEVO } = {};
            ...
            for (let i = 0; i < widgetsDesc.length; i++) {
                ...
                if (widgetsDesc[i].cons?.constructorName) {
                    ...
                    const attr: string = Handlebars.compile(attrTemplate)({
                        attr: obj.getAttrX({ keyCode: false }),
                    });
                    $(`#${cons}-attr`).html(attr);
                    ...
                    // listen to all possible widget events, not just those currently set for the widget -- the user may indicate more events in the editor panel
                    for (let evt in WidgetEvent) {
