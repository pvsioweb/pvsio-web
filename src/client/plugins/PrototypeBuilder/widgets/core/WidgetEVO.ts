...
export abstract class WidgetEVO extends Backbone.Model implements Widget {
        ...
        if (keys && keys.length) {
            ...
            for (let i in keys) {
                ...
                if (!opt.optionals.includes(key)) {
                    core[key] = this.attr[key];
                } else {
                    optionals[key] = this.attr[key];
                }
            }
    ...
    getAttrX (opt?: { keyCode?: boolean, optionals?: string[] }): WidgetAttrX {
        ...
        if (keys && keys.length) {
            const core: WidgetAttrX = {};
            const optionals: WidgetAttrX = {};
            for (let i in keys) {
                const key: string = keys[i];
                if (!opt.optionals.includes(key)) {
                    core[key] = { val: this.attr[key], hints: null };
                } else {
                    optionals[key] = { val: this.attr[key], hints: null };
                }
            }
            return {...core, ...optionals};
        };
    ...
    /**
     * Returns the list of events generated by this widget
     */
    getEvents (): WidgetEvent[] {
        const ans: WidgetEvent[] = [];
        if (this.evts) {
            const keys: WidgetEvent[] = <WidgetEvent[]> Object.keys(this.evts);
            for (let i in keys) {
                const evt: WidgetEvent = keys[i];
                if (this.evts[evt]) {
                    ans.push(evt);
                }
            }
