...
export type KnobEvent = BasicEvent | "rotate";
export type KnobEventData = {
    evt: KnobEvent,
    val: number, // current knob rotation, in deg
    fun: string // name of the prototype function to be invoked when a given event is triggered
};

export class KnobEVO extends ButtonEVO {
    protected rotation: number = 0;
    protected delta: number = 0;

    constructor (id: string, coords: Coords, opt?: ButtonOptions) {
        super(id, { width: 64, height: 64, ...coords }, opt);
