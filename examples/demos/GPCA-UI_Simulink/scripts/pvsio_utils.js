...
// this variables contains the GPCAUI state that will be rendered in the console.log window
	var pvsio_response;
...
// utility function, sends the command specified in ctrl_cmd to the GPCA controller
	function send_message_to_state_controller() {
		...
		logOnDiv("[GIPconsole] Sending message " + cmd + " to GPCA state controller...", 'console');
		gip.send(cmd);
	}
...
// utility function, renders the cursor in data entry mode
	...
	var read_gpcaui_state = function(e) {
		...
		logOnDiv("[ " + timestamp.toTimeString() + " ] ", 'pvsio_response_log');
		logOnDiv(pvsio_response, 'pvsio_response_log');
	...
	var render_text = function(display, text) {
	...
	client.addListener('WebSocketConnectionOpened', function () {
		...
		logOnDiv('PVSio Web Socket connected', 'console');
		/*
		 * Start the PVS Process for the pacemaker
		 */
		pvsio_websocket.startPVSProcess({
			name: 'gpcaUI.pvs',
			demoName: 'GPCA-UI_Simulink/pvs'
		}, function (err) {
			if (!err) {
				logOnDiv('PVS Process started', 'console');
				pvsio_websocket.sendGuiAction("gpcaui_init(0);", outputUpdated);
			} else {
		...
		console.console.log('web socket closed');
	}).addListener('processExited', function () {
		var msg = 'Warning!!!\r\nServer process exited. See console for details.';
		console.console.log(msg);
	});

	logOnDiv('Connecting to the PVSio server...', 'console');
	client.connectToServer();


	function outputUpdated(err, e){
	...
	//pvsio_websocket.serverUrl("ws://localhost:8080") //edit the server url here if not using ws://localhost:8080
	//	.addListener('ConnectionOpened', function(e){
	//		logOnDiv('WebSocket established', 'console');
	//		console.log("connection to server established");
	//		pvsio_websocket.startPVSProcess({
	//			name: 'gpcaUI.pvs',
	//			demoName: 'GPCA-UI_Simulink/pvs' //  "pvscode/gpcaui"
	//
	//		}, function (err) {
	//			if (!err) {
	//				logOnDiv('PVS Process started', 'console');
