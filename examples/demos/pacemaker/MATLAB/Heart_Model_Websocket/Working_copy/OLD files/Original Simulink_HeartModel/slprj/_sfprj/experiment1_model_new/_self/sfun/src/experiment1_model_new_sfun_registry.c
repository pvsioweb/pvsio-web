...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_experiment1_model_new_process_check_sum_call( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_experiment1_model_new_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_experiment1_model_new_autoinheritance_info( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  char commandName[64];
  ...
  if (strcmp(machineName, "experiment1_model_new") == 0) {
    const mxArray *newRhs[3] = { NULL, NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    newRhs[2] = prhs[3];
    return sf_experiment1_model_new_autoinheritance_info(nlhs,plhs,3,newRhs);
  }
...
{
  extern unsigned int sf_experiment1_model_new_get_eml_resolved_functions_info
  ...
  if (strcmp(machineName, "experiment1_model_new") == 0) {
    ...
    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_experiment1_model_new_get_eml_resolved_functions_info(nlhs,plhs,2,
      newRhs);
  }
...
extern unsigned int sf_experiment1_model_new_method_dispatcher(SimStruct *S,
...
unsigned int sf_machine_global_method_dispatcher(SimStruct *simstructPtr, const
  ...
  method, void *data)
{
  if (!strcmp(machineName,"experiment1_model_new")) {
    return(sf_experiment1_model_new_method_dispatcher(simstructPtr,
...
void sf_machine_global_terminator(void)
{
  if (sfMachineGlobalTerminatorCallable) {
    ...
    experiment1_model_new_terminator();
    sf_debug_terminate();
  }
...
extern void experiment1_model_new_register_exported_symbols(SimStruct* S);
extern void experiment1_model_new_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  experiment1_model_new_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfMachineGlobalInitializerCallable) {
    ...
    if (!simModeIsRTWGen) {
      experiment1_model_new_debug_initialize();
    }

    experiment1_model_new_initializer();
  }
